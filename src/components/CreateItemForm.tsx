import { useState, useCallback, useRef, useEffect, type ChangeEvent, type DragEvent, type FormEvent } from "react";
...
export default function CreateItemForm({ categories = [], onSubmit, serverErrors = {}, loading }: CreateItemFormProps) {
  ...
  function validate() {
    ...
    if (formData.quantity === "" || Number(formData.quantity) < 1)
      newErrors.quantity = "A quantidade deve ser no mínimo 1.";

    const price = parseFloat(String(formData.priceInCents));
    if (isNaN(price) || price <= 0)
      newErrors.priceInCents = "O preço deve ser maior que zero.";
  ...
  function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (Object.keys(localServerErrors).length > 0) {
      return
    }
    ...
    >
      ...
      <div className="flex items-center justify-end gap-3">
        <button
          ...
          className="w-full  max-w-[150px] h-12 px-6 py-3 rounded-lg border border-border text-sm font-medium text-text-muted hover:bg-surface-offset transition-colors cursor-pointer"
        >
          Cancelar
        </button>
        <button
          type="submit"
          className="w-full  max-w-[150px] h-12 px-6 py-3 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-light transition-colors cursor-pointer"
        >
          {loading ? <Spinner /> : "Salvar item"}
        </button>
      </div>
    </form>
  );
